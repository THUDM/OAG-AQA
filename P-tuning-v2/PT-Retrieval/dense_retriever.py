...
def validate(passages: Dict[object, Tuple[str, str]], answers: List[List[str]],
    ...
    return match_stats.questions_doc_hits, top_k_hits, match_stats.top_k_hits
...
def main(args):
    ...
    questions_doc_hits, top_k_hits, top_k_numbers = validate(all_passages, question_answers, top_ids_and_scores, args.validation_workers,
                                  args.match, check_index)
    topic = args.qa_file.split('/')[-1].split('-')[0]
    if not os.path.exists(args.out_file):
        result = {}
        result[topic] = {
        "top_k_hits": top_k_hits,
        "top_k_numbers": top_k_numbers
        
        }
        with open(args.out_file, 'w') as fw:
            fw.write(json.dumps(result, indent=4))
    else:
        with open(args.out_file, 'r') as fr:
